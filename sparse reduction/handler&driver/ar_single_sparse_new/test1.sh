...
for hosts in 8 16 32; do
    for blocks in 8 16 32; do
        for sparse in 1 2 8; do
            echo $hosts $blocks "int32 ar_single_sparse array" $sparse
            ...
            target=$(cat transcript | tail -20 | head -12 | grep -Eo '[0-9\.]+' | tr "\n" " ")
            echo $hosts $blocks "int32 ar_single_sparse array" $sparse $target  >> result.csv
...
for hosts in 32; do
    for blocks in 8 16 32; do
        for sparse in 1 2 8 32 128; do
            ...
            echo $hosts $blocks "int32 ar_single_sparse hash" $sparse
            ./sim_ar_single_sparse > transcript
            target=$(cat transcript | tail -20 | head -12 | grep -Eo '[0-9\.]+' | tr "\n" " ")
            echo $hosts $blocks "int32 ar_single_sparse hash" $sparse $target  >> result.csv
