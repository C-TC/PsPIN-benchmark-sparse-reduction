    ...
    for blocks in 8 16 32; do
        for sparse in 2 8; do
            for streams in 2 3 4; do
                ...
                echo $hosts $blocks "int32 ar_single_sparse array" $sparse $streams
                ./sim_ar_single_sparse > transcript
                target=$(cat transcript | tail -20 | head -12 | grep -Eo '[0-9\.]+' | tr "\n" " ")
                echo $hosts $blocks "int32 ar_single_sparse array" $sparse $streams $target  >> result.csv
            done
        done
    done
done
    ...
    for blocks in 8 16 32; do
        for sparse in 2 8; do
            for streams in 2 3 4; do
                ...
                echo $hosts $blocks "int32 ar_single_sparse hash" $sparse $streams
                ./sim_ar_single_sparse > transcript
                target=$(cat transcript | tail -20 | head -12 | grep -Eo '[0-9\.]+' | tr "\n" " ")
                echo $hosts $blocks "int32 ar_single_sparse hash" $sparse $streams $target  >> result.csv
            done
        done
    done
done
